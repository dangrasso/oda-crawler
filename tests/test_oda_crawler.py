...
class UtilTest(unittest.TestCase):
    def test_collect_product(self):
        ...
        with open(f"{FIXTURES_PATH}/product_chicken.html") as f:
    ...
    def test_collect_product__with_no_categories(self):
        products = []
        crawler = OdaCrawler("dummy.url", 1, products=products)
        with open(f"{FIXTURES_PATH}/product_no_categories_nellik.html") as f:
            content = BeautifulSoup(f, "html.parser")
    ...
    def test_collect__other(self):
        products = []
        crawler = OdaCrawler("dummy.url", 1, products=products)
        content = BeautifulSoup("<html>Sorry mate, wrong page!</html>", "html.parser")
